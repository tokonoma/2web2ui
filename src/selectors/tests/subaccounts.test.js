...
describe('Subaccount selectors', () => {
  describe('selectSubaccount', () => {
    it('should return subaccount unformatted', () => {
      ...
      expect(selector.selectSubaccount(state)).toEqual({
        compliance: false,
        status: 'foo',
        key: 'value',
      });
    ...
    it('should return compliance_status as status when it is not active', () => {
      const state = {
        subaccounts: { subaccount: { compliance_status: 'terminated', key: 'value' } },
      };
      expect(selector.selectSubaccount(state)).toEqual({
        compliance: true,
        status: 'terminated',
  ...
  describe('selectSubaccounts', () => {
    ...
    it('should return false if user is a subaccount user', () => {
      const state = {
        currentUser: { has_subaccounts: true, access_level: 'subaccount_reporting' },
      };
      expect(selector.hasSubaccounts(state)).toEqual(false);
    });
  ...
  describe('getSubaccountsIndexedById', () => {
    it('should return the subaccounts indexed by id', () => {
      ...
      expect(selector.getSubaccountsIndexedById(state)).toEqual({
        101: { name: 'sub 1', id: 101 },
