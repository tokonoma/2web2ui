...
const asyncValidate = async ({ csv, ...values }) => {
  ...
  // see, https://stackoverflow.com/a/5515960
  // note, config.maxUploadSizeBytes is wrong its 20mb, should be 10mb
  const recipients = await parseRecipientListCsv(csv);
  const json = JSON.stringify({ ...values, recipients });
  const m = encodeURIComponent(json).match(/%[89ABab]/g);
  const size = json.length + (m ? m.length : 0);
...
const WrappedForm = reduxForm({ form: formName, asyncValidate })(RecipientListForm);
