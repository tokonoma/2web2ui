...
describe('SendTestEmailButton', () => {
  it('opens the modal and saves when the "Send a Test" button is clicked', () => {
    ...
    useEditorContext.mockReturnValue({
      ...
      updateDraftV2: jest.fn(() => promise),
      sendPreviewV2: jest.fn(() => promise),
      showAlert: jest.fn(),
      setHasSaved: jest.fn()
    });
    const { getByText } = render(<SendTestEmailButton/>);

    fireEvent.click(getByText('Send a Test'));

    return promise.then(() => {
      expect(getByText('Verify your email renders as expected in the inbox by sending a quick test.')).toBeInTheDocument();
    });
  });
});
