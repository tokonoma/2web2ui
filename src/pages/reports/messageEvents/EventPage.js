...
export class EventPage extends Component {
  ...
  render() {
    ...
    if (this.state.shouldRedirect) {
      const errorMessageInfo = isOrphanEvent
      ...
      return (
        <RedirectAndAlert
          ...
          alert={{
            type: 'warning',
            message: `Unable to find event(s) data with ${errorMessageInfo}`,
          }}
        />
      );
    }
    ...
    return (
      <Page title={title} breadcrumbAction={breadcrumbAction}>
        {pageContent}
      </Page>
    );
  }
...
export default connect(eventPageMSTP, { getMessageHistory, getDocumentation, getSelectedEvent })(
  EventPage,
);
