...
const ALWAYS_SHOW_ON_PATH = ['/account/billing', '/account/settings'];
...
export class GlobalBanner extends Component {
  render() {
    ...
    const value = {
      bannerOpen: Boolean(
        showBanner &&
          ...
          (moment.duration(moment(pending_cancellation.effective_date).diff(moment.utc(), 'days')) <
            7 ||
    ...
    return <BannerContext.Provider value={value}>{this.props.children}</BannerContext.Provider>;
  }
}
